...
| **前置条件** | 无 |
| **测试步骤** | 1. 访问 `/register.html`<br>2. 依次输入 `a` / `abc123` / `Abc12345` / `Abc!12345` 到 `input[name="password"]`<br>3. 每次观察 `#strength[data-score]` 与 `.strength__label` 文本 |
| **预期结果** | 1. 输入 1 字符：`data-score="0"`，label 为空<br>2. 输入 6 字符纯字母数字：`data-score="2"`，label "一般"<br>3. 输入 10 字符混合大小写字母与数字：`data-score="3"`，label "良好"<br>4. 输入含特殊符号：`data-score="4"`，label "很强" |
...
> **前置约定**：TC-027 ~ TC-038 全部基于"`admin` 已登录"前置；测试题目使用后端初始化脚本中的样例题（如有"两数之和" `id=1`）。若题库为空，请先通过 TC-040 ~ TC-043 创建至少一道"两数之和"题。
